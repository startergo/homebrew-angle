...
awk '
  ...
  in_template && in_shared_lib && in_configs && /\]/ {
    ...
    print $0
    print "    if (is_apple) {"
    print "      ldflags = [ \"-Wl,-headerpad_max_install_names\", \"-Wl,-install_name,@rpath/libGLESv2.dylib\" ]"
    print "    }"
    modified = 1
    next
  }
...
awk '
  ...
  in_template && in_libegl_template && in_data_deps && /\]/ {
    ...
    print $0
    print "    if (is_apple) {"
    print "      ldflags = [ \"-Wl,-headerpad_max_install_names\", \"-Wl,-install_name,@rpath/libEGL.dylib\" ]"
    print "    }"
    modified = 1
    next
  }
...
awk '
  ...
  in_target && in_configs && /\]/ {
    ...
    print $0
    print "  if (is_apple) {"
    print "    ldflags = [ \"-Wl,-headerpad_max_install_names\", \"-Wl,-install_name,@rpath/libGLESv1_CM.dylib\" ]"
    print "  }"
    modified = 1
    next
  }
